...
SET NOCOUNT ON
        ...
        DECLARE @SkupnaRazlika DECIMAL(24, 8) ;
...
--        SET @Razlika = @SaldoOsnovnihObracunov - @CenaIzravnave
        ...
        DECLARE @MaxRazlika DECIMAL(24, 8) ;
        DECLARE @MaxInterval DATETIME ;
        DECLARE @ExitCondition INT = 0 ;
        DECLARE @C_Interval DATETIME ;
        DECLARE @C_Razlika DECIMAL(24, 8) ;
        DECLARE @C_SkupnaOdstopanja DECIMAL(24, 8) ;
        ...
        DECLARE @CpNov DECIMAL(24, 8) ;
        DECLARE @CmNov DECIMAL(24, 8) ;
        DECLARE @C_Cp DECIMAL(24, 8) ;
        DECLARE @C_Cm DECIMAL(24, 8) ;

        DECLARE @Qp DECIMAL(24, 8) ;
        DECLARE @Qm DECIMAL(24, 8) ;
        DECLARE @C_StrIzravnave DECIMAL(24, 8) ;
        DECLARE @C_SaldoStroski DECIMAL(24, 8) ;
        DECLARE @C_Wgjs_p DECIMAL(24, 8) ;
        DECLARE @C_Wgjs_m DECIMAL(24, 8) ;
        DECLARE @C_Wpi DECIMAL(24, 8) ;
        DECLARE @C_Wmi DECIMAL(24, 8) ;
        DECLARE @C_SIPXurni DECIMAL(24, 8) ;
            ...
            BEGIN
                ...
                IF ( @NetoOdstopanjaPozitivna = 1
                    ...
                    BEGIN
                       ...
                       -- SET @Razlika = ABS(@SkupnaRazlika)
                        ...
                        SET @Qp = ABS(@SkupnaRazlika) / ( @C_Wpi + ( @C_Wgjs_p / 1.03 ) )
                        SET @CpNov = @Qp + @C_Cp ;
                ...
                ELSE 
                    IF ( @NetoOdstopanjaPozitivna = 1 ) 
                        BEGIN
                            SET @CpNov = @C_Cp ;	
                            SET @CmNov = @C_Cm ;
                            SET @Qp = ( @C_StrIzravnave - @C_SaldoStroski )
                                / ( @C_Wpi + ( @C_Wgjs_p / 1.03 ) )
                            SET @CpNov = @Qp + @C_Cp ;
							SET @Qm = 0;
                ...
                IF ( @NetoOdstopanjaNegativna = 1
                    ...
                    BEGIN
                        ...
                        --SET @Razlika = ABS(@SkupnaRazlika)
                        --PRINT 'RAZLIKA ZADNJI 1='+CAST(@SkupnaRazlika AS varchar(20));
                        SET @Qm = ABS(@SkupnaRazlika) / ( -@C_Wmi - ( @C_Wgjs_m / 0.97 ) )
                        SET @Qp = 0;
                        SET @CmNov = -1*@Qm + @C_Cm ;
                ...
                ELSE 
                    IF ( @NetoOdstopanjaPozitivna = 1 ) 
                        ...
                        PRINT 'NovaCenaOdsNegPrimankljaj INTERVAL = '+CAST(@C_Interval AS VARCHAR(24))
                            SET @CpNov = @C_Cp ;	
                            SET @CmNov = @C_Cm ;
                            SET @Qm = ( -@C_StrIzravnave + @C_SaldoStroski )
                                / ( @C_Wmi + ( @C_Wgjs_m / 0.97 ) )
                            SET @Qp = 0;
                            SET @CmNov = @Qm + @C_Cm ;
                            IF (@CmNov < 0) SET @CmNov = 0;
                ...
                IF ( @NetoOdstopanjaNegativna = 1
                    ...
                    BEGIN
                        ...
                        SET @Qp = @SkupnaRazlika / ( -1 * @C_Wmi - ( @C_Cm / 0.97 ) ) 
                        ...
                        IF ( @CmNov < 0 ) 
                            ...
                            SET     PodatkiObracuna_Skupni.SaldoStroskiObracunov = ISNULL(O.Saldo, 0),
								    PodatkiObracuna_Skupni.Razlika = ISNULL(O.Saldo, 0) -[dbo].[PodatkiObracuna_Skupni].SroskiIzravnave 
								    --[dbo].[PodatkiObracuna_Skupni].[PreostalaVrednost] = 
                            FROM    ( SELECT    SUM(CASE WHEN R.[Odstopanje] > 0
                                                         THEN R.[Odstopanje] * @CpNov / ( CASE WHEN OZT.Sifra = 'GJS' THEN 1.03
                                                                                                 ELSE 1
                                                                                            END )
